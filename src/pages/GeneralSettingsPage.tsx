...
export default function GeneralSettingsPage() {
  ...
  const { allRoles, isAdmin, approveUser, rejectUser } = useUserRole();
  ...
  const handleChangePassword = async () => {
    if (!changePassValue.trim()) return;
  ...
  return (
    <div className="animate-slide-in max-w-2xl">
      <h1 className="text-3xl font-display font-bold text-foreground mb-1">Configurações Gerais</h1>
      <p className="text-muted-foreground mb-8">Aparência, conta e gestão de equipe</p>
              ...
              <div key={m.id} className="p-3 bg-secondary rounded-lg space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {isAdmin && !isMe && (
                      <>
                        <Button variant="ghost" size="sm" onClick={() => openChangePassword(m.user_id, m.full_name)}
                          className="h-8 w-8 p-0 text-muted-foreground hover:text-primary hover:bg-primary/10" title="Alterar senha">
                          <KeyRound className="w-4 h-4" />
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => openDeleteUser(m.user_id, m.full_name)}
                ...
                {isAdmin && (
                  <Input
                    ...
                    onBlur={(e) => { if (e.target.value !== m.job_title) handleUpdateJobTitle(m.id, e.target.value); }}
          ...
          <div className="border-t border-border pt-4">
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                <Input type={showNewPassword ? 'text' : 'password'} placeholder="Senha (mín. 6 caracteres)"
              ...
              <div className="relative">
                <Input type={showNewPassword ? 'text' : 'password'} placeholder="Confirmar senha"
                  value={newPasswordConfirm} onChange={e => setNewPasswordConfirm(e.target.value)} minLength={6} />
                <button type="button" onClick={() => setShowNewPassword(!showNewPassword)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
                  {showNewPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                </button>
              ...
              <Select value={newRole} onValueChange={(v) => setNewRole(v as 'admin' | 'member')}>
                <SelectTrigger className="h-9 text-sm"><SelectValue placeholder="Tipo de acesso" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="member">Membro</SelectItem>
                  <SelectItem value="admin">Admin</SelectItem>
                </SelectContent>
        ...
        <DialogContent>
          ...
          <div className="mt-2 space-y-5">
            <p className="text-sm text-muted-foreground">
              Você está prestes a excluir <strong className="text-foreground">{deleteUserName}</strong> permanentemente.
            </p>
            <div>
              ...
              <Input value={deleteUserConfirm} onChange={e => setDeleteUserConfirm(e.target.value)} placeholder="EXCLUIR" className="font-mono" />
            </div>
            <div className="flex gap-3">
              <Button variant="outline" onClick={() => setDeleteUserOpen(false)} className="flex-1">Cancelar</Button>
              <Button variant="destructive" onClick={handleDeleteUser} disabled={deleteUserConfirm !== 'EXCLUIR' || deleting} className="flex-1">
      ...
      <Dialog open={changePassOpen} onOpenChange={setChangePassOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="font-display">Alterar Senha — {changePassUserName}</DialogTitle>
          </DialogHeader>
          <div className="mt-2 space-y-4">
            <div className="relative">
              <Input type={showChangePass ? 'text' : 'password'} placeholder="Nova senha (mín. 6 caracteres)"
                value={changePassValue} onChange={e => setChangePassValue(e.target.value)} minLength={6} />
            ...
            <div className="relative">
              <Input type={showChangePass ? 'text' : 'password'} placeholder="Confirmar nova senha"
                value={changePassConfirm} onChange={e => setChangePassConfirm(e.target.value)} minLength={6} />
              <button type="button" onClick={() => setShowChangePass(!showChangePass)}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
                {showChangePass ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
              </button>
      ...
      <div className="mt-12 pt-8 border-t border-border/50 flex flex-wrap gap-x-6 gap-y-2 opacity-50 hover:opacity-100 transition-opacity">
        <a href="/privacy" className="text-[10px] text-muted-foreground hover:text-primary transition-colors underline-offset-4 hover:underline">Política de Privacidade</a>
        <a href="/terms" className="text-[10px] text-muted-foreground hover:text-primary transition-colors underline-offset-4 hover:underline">Termos de Uso</a>
        <p className="text-[10px] text-muted-foreground ml-auto">© 2026 PostFlow</p>
      </div>
    </div>
  );
}
